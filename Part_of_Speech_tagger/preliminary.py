...
def train(data_path):
    ...
    for sample in read_jsonl(data_path):
        ...
        YOUR CODE HERE
        '''
        # Set model model params to be a dictionary with tag counts.
...
def predict_tag(token, params):
    ...
    tag_freq = 0
    for token in params:
        for tag in params[token]:
            if params[token][tag] > tag_freq:
                tag_freq = params[token][tag]
    ...
    # Output.
    for tag in most_freq[token]:
        tag_pred = tag
        tag_count = most_freq[token][tag]
    # print(token)
    # print(tag_pred)
    # print(tag_count)
...
def predict(x, params):
    ...
    """
    # 让x总是以列表形式输入.
    is_string = type(x) == str   # x是string，则返回True
    if is_string:  # 如果x是string，则把它装到列表中
        x = [x]
