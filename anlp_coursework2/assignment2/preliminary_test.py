...
def train(data_path):
    ...
    my_params = defaultdict(lambda : defaultdict(int))
    for sample in read_jsonl(data_path):
        sample_count += 1
        tokens = sample['tokens']   # token -> list 
        tags = sample['tags']
        
        for i in range(len(tokens)):
            my_params[tokens[i]][tags[i]] +=1
            my_params['_tags_'][tags[i]] +=1
...
def predict_tag(token, params):
    ...
    '''
    if token in params:
        tag_pred = max(params[token], key=params[token].get)
        tag_count = params[token][tag_pred]
    else:
        tag_pred = max(params['_tags_'], key=params['_tags_'].get)
        tag_count = 0
