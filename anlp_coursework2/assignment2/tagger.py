...
def evaluate(model_path, data_path, split='TESTING'):
    ...
    for i in range(len(tokens_list)):
        if 'Brazil' in tokens_list[i]:
            continue
        elif i == 1187:
            continue
        else:
            for j in range(len(tokens_list[i])):
                wordlist.append(tokens_list[i][j])
    # print(len(labels_))
    # print(len(wordlist))
    with open('now_results.txt', 'w', encoding='utf-8') as sample:
        for i in range(len(labels_)):
            if labels_[i] != predictions_[i]:
                sample.write('token: {:<10} tag: {:<10} prediction: {:<10}    #FALSE!\n'.format(wordlist[i],labels_[i],predictions_[i]))
                # print('token: '+wordlist[i]+'  '+'tag: '+labels_[i]+'  '+'prediction: '+predictions_[i]+'    #FALSE!')
            else:
                sample.write('token: {:<10} tag: {:<10} prediction: {:<10} \n'.format(wordlist[i], labels_[i],predictions_[i]))
                # print('token: '+wordlist[i] + '  ' + 'tag: ' + labels_[i] + '  ' + 'prediction: ' + predictions_[i])
